...
if($user_type=='admin'){
    $sql ="SELECT * FROM cities ORDER BY status";
    ...
    $sql ="SELECT cities.* FROM cities INNER JOIN users ON  cities.added_user = users.id WHERE
    users.id=$user_id "; 
    $query= $conn -> prepare($sql);
...
<head>
    ...
    <title>Hire My Developer | Cities</title>
    <!-- Footable CSS -->
                                            ...
                                            <tr>
												...
												<td>
													<?php 
														if($result['image']){
														?>
														<img src="./uploads/cities/<?php echo $result['image']; ?>" alt="No Image" />
														<?php
														}		
													?>
												</td>
                                                ...
                                                <td><?php echo $result['city_name']?></td>
												 ...
												 <td><?php echo $result['city_decription']?></td>
                                             <!-- 
												<td>
											...
											-->
                                                ...
                                                <?php
                                                        if($user_type=='admin') {
                                                            if ($result['added_user']==1){
                                                                if($result['status']==0){ ?>
                                                                    <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-success" onclick="changeCityStatus('<?php echo $result['id'] ?>',1);" >Approve</a>
                                                                <?php } else if($result['status']==1) { ?>
                                                                    <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-danger" onclick="changeCityStatus('<?php echo $result['id'] ?>',2);" >Disable</a>
                                                                <?php }
                                                                else{?>
                                                                     <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-success" onclick="changeCityStatus('<?php echo $result['id'] ?>',1);" >Approve</a>
                                                               <?php }              
                                                                ...
                                                                if($result['status']==0){ ?>
                                                                    <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-success" onclick="changeCityStatus('<?php echo $result['id'] ?>',1);" >Approve</a>
                                                                <?php } else if($result['status']==1) { ?>
                                                                    <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-danger" onclick="changeCityStatus('<?php echo $result['id'] ?>',2);" >Reject</a>
                                                                <?php }
                                                                else{?>
                                                                     <a href="javascript:void(0)" class="btn btn-xs btn-rounded btn-outline-success" onclick="changeCityStatus('<?php echo $result['id'] ?>',1);" >Approve</a>
                                                               <?php } 
                                                    ...
                                                    if($user_type!='admin') {
                                                        if($result['status']==0) {?>
                                                   <a href="manage_cities.php?id=<?php echo $result['id']?>">													
													<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline" data-toggle="tooltip" data-original-title="Edit"><a href="manage_cities.php?id=<?php echo $result['id'] ?> "><i class="ti-pencil" aria-hidden="true"></i></button>
                                                    </a>
													
                                                    <button type="button" onclick="deleteCity('<?php echo $result['id']; ?>');" class="btn btn-sm btn-icon btn-pure btn-outline" data-toggle="tooltip" data-original-title="Delete"><i class="ti-close" aria-hidden="true"></i></button>
                                                        <?php }
                                                    }
                                                    else{?>
                                                        <a href="manage_cities.php?id=<?php echo $result['id']?>">													
      ...
      function deleteCity(id) {
        ...
        if (r == true) {   
         // alert('City deleted Successfully !');
          document.location.href='delete_city.php?dlt_id='+id;
        } 
     
      }
	  function changeCityStatus(id,flag) {
        var r = confirm("Do you want to change the status !");
        if (r == true) {   
          //alert('City Status Changed Successfully !');
          document.location.href='status_city.php?id='+id+'&flag='+flag;
        } 
